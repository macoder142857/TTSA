...
for i=1:30%测试哪个函数开关
    ...
    for j=1:runs %每个函数测试多少趟开关，一般为30趟
        ...
        [TTSA_func(i,j).gbest,DE_TSA_gbestVal2(i,j),TTSA_func(i,j).convergence, TTSAtime2(i,j)]  = DTSA_func(fhd,D,pop_size,iter_max,Xmin,Xmax,func_num);

        convergenceData{i}{j, 1} = TSA(i,j).convergence;
        convergenceData{i}{j, 6} = TTSA_func(i,j).convergence;
    end
     ...
     f_mean_DE_TSA2(i)=mean(DE_TSA_gbestVal2(i,:)); 
      TTSA_func(i).convergMean=mean(cat(1,TTSA_func(i,:).convergence)); %记录TSA收敛曲线
     f_std_DE_TSA2(i)=std(DE_TSA_gbestVal2(i,:)); 
     f_mean_DE_TSAtime2(i)=mean(TTSAtime2(i,:));
  ...
  figure(i)
    ...
    hold on
    semilogy(TTSA_func(i).convergMean,'Color', 'r');
    hold on
    indices = 1:50:500;
    plot(indices, TTSA_func(i).convergMean(indices), 'r*'); % 标记红色圆点
